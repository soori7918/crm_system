        ...
        <tbody>
            @foreach($return_items ?: [] as $key => $item)
                <tr>
                    <td>{{ $key+1 }}</td>
                    <td>
                        <span class="item_id" data-item-id="{{ $item->item_id }}"></span>
                    ...
                    <td>
                        <form method="post" action="{{route('panel.product_change.delete_item' , $item->id)}}">
                            @csrf
                            @method('delete')
                        ...
                        {{-- <form action="{{ route('panel.returnProductChange.delete_return_items')}}" method="post">
                            @csrf
